...
ebpf_load_elf(struct ebpf_vm *vm, const void *elf, size_t elf_size)
{
	...
	for (i = 0; i < ehdr->e_shnum; i++) {
		...
		const void *data =
		    bounds_check(&b, shdr->sh_offset, shdr->sh_size);
		if (!data) {
	...
	for (i = 0; i < ehdr->e_shnum; i++) {
		...
		for (j = 0; j < rel->size / sizeof(Elf64_Rel); j++) {
			...
			if (ELF64_R_TYPE(r->r_info) != 2) {
				ebpf_error("bad relocation type %u",
			...
			unsigned int imm =
			    ebpf_lookup_registered_function(vm, sym_name);
			if (imm == -1) {
				ebpf_error("function '%s' not found", sym_name);
				goto error;
			}

			*(uint32_t *)((uint8_t *)text_copy + r->r_offset + 4) =
			    imm;
		}
	}
